  ...
  and measures for you.'''
    ...
    def handle(self, *args, **options):
        ...
        for contest_item in out_list:
            wrapper, position = ('', '')
            try:
    #             wrapper, position = contest_item[0].split(',', 1)
    ...
    #             party_code = contest_item[1][0][2], # only if a primary
    # #             district_category = contest_item[1][0][9],
                region_id = 4, # Just default to Lane County
                print_only = True,
            )
            new_contest.save()
            c = new_contest.id
